...
interface FetchApiOptions extends Omit<AxiosRequestConfig, 'url' | 'method' | 'headers' | 'data'> {
...
if (!API_KEY) {
  ...
  console.error('Por favor, crea un archivo .env y añade STABILITY_API_KEY=tu_clave_de_api');
  process.exit(1);
}
...
export async function fetchApi<T = any>(url: string, options: FetchApiOptions = {}): Promise<T> {
  try {
...
async function generateVideo(): Promise<void> {
  ...
  if (DRY_RUN) {
    ...
    console.log('La configuración es correcta, pero no se llamará a la API para evitar costos.');
    console.log('Para generar el video, elimina o establece DRY_RUN=false en el archivo .env');
    return;
  }
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      console.log(`Verificando estado de generación (Intento ${attempts + 1}/${maxAttempts})...`);
      ...
      if (statusResponse.status === 'succeeded') {
        ...
        throw new Error(`La generación del video falló: ${statusResponse.error || 'Error desconocido'}`);
      }
    ...
    if (attempts >= maxAttempts) {
      throw new Error('Tiempo de espera agotado. La generación del video está tomando más tiempo de lo esperado.');
    }
...
export const putImage = async (filePath: string, metadata: { filename: string, contentType: string }) => {
  let tempFilePath = '';
